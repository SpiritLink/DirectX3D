...
LPD3DXMESH cRawLoader::Load(IN char * szFolder, IN char * szFile)
{
	...
	for (int i = 0; i < RAWSIZE; ++i)
	{
		...
		{
			p.p.x = i;
			p.p.y = m_vecRaw[i + j * RAWSIZE] / 10.0f;
			p.p.z = j;

			if (i == 0)
				p.t.x = 0;
			else
				p.t.x = i / (float)RAWSIZE;

			if (j == 0)
				p.t.y = 0;
			else
				p.t.y = j / (float)RAWSIZE;
	...
	for (int i = 0; i < TILESIZE; ++i)		///x
	{
		...
		{
			WORD k;
			k = i + j * TILESIZE;					m_vecIndex.push_back(k);	///왼쪽 아래
			k = i + j * TILESIZE + 1 + TILESIZE;	m_vecIndex.push_back(k);	///오른쪽 위
			k = i + j * TILESIZE + 1;				m_vecIndex.push_back(k);	///오른쪽 아래

			k = i + j * TILESIZE;					m_vecIndex.push_back(k);	///왼쪽 아래
			k = i + j * TILESIZE + TILESIZE;		m_vecIndex.push_back(k);	///왼쪽 위
			k = i + j * TILESIZE + 1 + TILESIZE;	m_vecIndex.push_back(k);	///오른쪽 위
		}
	}

	m_vecAttribute.resize(m_vecIndex.size() / 3, 0);
